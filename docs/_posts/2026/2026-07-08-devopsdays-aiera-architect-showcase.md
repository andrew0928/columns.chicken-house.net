...
今年六月底, 忙完兩場演講 ([DevOpsDays Taipei 2026 Keynote](https://devopsdays.tw/2026/session/4702), [Generative AI](https://gaiconf.com/agenda) 開發者年會 2026), 把我上半年的所有研究都交代完了｡ 過程中得到不少現場參加的朋友給我的回饋, 最熱烈的是 DevOpsDays 會後, 找我聊我示範的 side project: Andrew Shop 的執行方法｡ 其實這本來只是單純的架構設計難題驗證而已 (交易系統, 要標準化, 又必須滿足高度客製化的要求), 只是我 "順手" 用了我覺得 AI coding 應該要有的樣子來執行而已｡
...
從這邊開始, 我會引用多張我在 DevOpsDays 台上講的投影片來說明 "架構師" 的注意力有多重要, 也會包含部分只有在現場提及的內容｡ 我在 DevOpsDays 這場 Keynote, 談的範圍不只是這個開發案例, 另外也包括了團隊導入 AI 之後的瓶頸分析, 以及工作方式的改變｡ 我提及了 AI 時代, 軟體開發團隊自身最重要的三件事: Core Context / Core Workflow / Core Cycle, 其實就是對應到 Harness Engineering 的工作方法; 而這篇講的委派任務給 agent 處理的基本結構, 就是我在這場演講提及的 Core Cycle.

這篇文章以下的部分, 我開始會綜合這場演講的內容一起總結了, 如果你覺得有些內容有點突兀, 跟前面接不起來, 可以先回頭翻一下我釋出的 [簡報](https://slides.chicken-house.net/s/2026-DEVOPSDAYS) 跟 [大會共筆](https://hackmd.io/@DevOpsDay/2026/%2FByXqCQt-Gl) 當作參考｡

我在開場用的這頁簡報:

![alt text](/images/2026-07-08-devopsdays-aiera-architect-showcase/image-6.png)

這頁說明的是: 將複雜系統 (大黑箱) 深度拆解的效益, 當你想一次解決大黑箱, 需要花費的成本, 通常都跟黑箱的規模的二次方 ~ 三次方成正比, 這是經驗法則, 因此如果你有能力切割, 切的越精確通常效益會越好 (微服務就是一種典型的例子, 亂切除外)｡

切的面向也有區別, 除了橫向的切割 (通常是按照功能, 以這次案例而言, 整個系統拆出計費機制, 拆出額度管控, 只是其中一個功能), 也包含縱向切割 (通常是按照階層, 以這次案例而言, 分成 api 要處理的非功能性需求, 以及 .core 要處理的業務邏輯, 以及 .abstract 要處理的介面規格), 這些一路切下去, 你可以把大黑箱切成多個小黑箱, 只要你切的夠細緻, 你不需要完整實作你就能掌握系統全貌...
...
這段我點出了最關鍵的一點: 影響 agent 能力的因素, 大致可以分兩類, 一個是模型跟工具, 屬於外面買的到的, 大家都差不多 (除了付不起 token 的例外), 我測試用的 codex + gpt5.5, claude code + fable5 都是大家有訂閱就能用的東西; 而真正有差異的, 是內部團隊的準備程度. 我在這篇探討的都是規格, 但是實際上還有 context 的掌握能力 (這我這篇沒有談到), 以及如何有效率的圍繞著 context 協作的工作流程都算｡ 這些的成熟度, 才是真正的能力護城河｡
