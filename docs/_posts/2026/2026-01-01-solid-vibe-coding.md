...
AI 工具越來越厲害, 但是也越來越難掌控, 所以我一直想寫這個主題, 用架構師的角度來思考該怎麼駕馭這神兵利器｡ 準備的過程中, 剛好看了兩篇文章蠻有感的, 一個是 OpenClaw 作者 Peter Steinberger 的專訪: [I ship code I don't read](https://newsletter.pragmaticengineer.com/p/the-creator-of-clawd-i-ship-code), 另一個是高見龍最近寫了一篇 [Spec-as-source 的理想與現實](https://kaochenlong.com/sdd-spec-as-source) ... 這兩篇文章都說中我的核心論述: SDD 很有效, 但是本質上就是 waterfall, 改了規格許多步驟需要重來一次, 然而 ai 有不確定性, 再來一次可能結果又不同了, 因此 SDD 高度依賴 SPEC 的品質, 而 SDD 會有效, 另一個原因是 agent 的能力大幅降低 SDD 的門檻, 讓這套過去難以實現的流程現在能很快的轉起來｡ 在 Peter 的專訪提到, 開發速度越來越快, 你不可能用人力的 code review 來跟上 AI 的速度了, 因此想盡各種方法降低你 "必須" review 的範圍跟確保品質的方法越來越重要 (簡單的說就是創造不需要 "人工" review 的工作方法)｡ 越複雜的系統, 越需要在一開始就把架構設計做好, 越前面階段就掌握好品質, 後面的速度就會越快, 這是不變的道理｡

所以, 我挑了個過去嘗試過的 side project: 折扣計算 當作案例, 用 ai 重做了一次, 目的就是想驗證上面的想法｡ 我把構想分兩階段, 第一是想法如何變成精確的 SPEC, (架構師的任務通常都不會有人給你 "明確" 的 SPEC), 有了好的 SPEC 後, 接著用 SDD 才會派上用場｡ 而好的 SPEC 該怎麼寫? 光是按照 Spec-Kit 我覺得是不夠的, 我碰過複雜度很高的案子, speckit - clearify 步驟時, 你可能連 ai 問的問題都回答不出來..., 這時若你能用 code 來描述 SPEC 的關鍵部分, 那很多 SPEC / Docs 維護的問題瞬間就變很簡單了
...
這些做法, 可以讓規格能被系統化的驗證, 大幅降低人跟 AI 的認知負擔, 免除了很多讓 AI 自由發揮的空間, AI 可以直接用 compiler / tests 就能確認規格是否正確被實作, 我可以避開非常多 "無聊" 的 code review, 把更多時間花在前期的設計上。其實這並不是什麼新觀念, 這是架構師行之有年的工作方法, 只是過去我用在跨團隊溝通，現在我把它拿來跟 AI 溝通而已。高見龍文章裡提到的「形式方法」就是這個思路的極致版本，而 TypeScript 的型別系統、Rust 的所有權機制，都是「輕量版形式方法」的成功案例。

這篇文章，我用一個實際案例來示範這套工作流: **Contract-First + TDD + SDD**, 在看這個實際操作案例前, 各位可以先思考這幾個問題:
...
# 2. Contract-First：用 Code 當 Spec
...
# 4. 實作過程：架構師主導的 Contract-First 流程
...
## 4.1 測試先行：用 Decision Table 確保規格完備
...
經過這次實驗，我把對照組與實驗組的差異整理成下表：

| 面向 | 對照組（純 Vibe Coding） | 實驗組（Contract-First） |
|------|-------------------------|------------------------|
| **介面設計** | AI 自行決定，事後才知道長什麼樣 | 人工定義 interface，編譯器驗證 |
| **測試範圍** | AI 自行決定要測什麼 | 用 Decision Table 系統化規劃 |
| **測試數量** | 24 個（AI 自己想的） | 25 個（覆蓋所有條件組合） |
| **架構掌控** | 黑箱，只能事後 review | 關鍵介面事前定義，其餘交給 AI |
| **規格變更成本** | 高（牽一髮動全身） | 低（介面穩定，實作可重寫） |
| **跨團隊協作** | 難（介面沒有共識） | 易（interface 就是合約） |
| **可維護性** | 取決於 AI 當下的決定 | 架構師定義骨架，AI 填肉 |
| **開發時間** | 5 premium requests | 25 premium requests |
| **品質信心** | 只能相信 AI | 編譯器 + 測試 + debugger 驗證 |

**核心差異在於：誰決定 contract？誰驗證 contract？**

對照組把這兩件事都交給 AI，你只能事後接受結果；實驗組則是人工定義 contract、用編譯器和測試來驗證，AI 只負責在明確的框架內填寫實作細節。

花 5 倍的時間值得嗎？這取決於你的專案性質：
- 如果是一次性的 prototype 或小工具，對照組的做法完全夠用
- 如果是需要長期維護、跨團隊協作的大型系統，實驗組的投資會在後續持續回收
