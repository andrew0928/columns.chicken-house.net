...
要讓 AI 寫出能動的 code 已經不是什麼新鮮事了, 不過務實上, 大型系統的開發, "寫出能動的 code" 只是一小部分, 更重要的還有跨團隊協作的介面規格、可驗證的測試覆蓋、以及長期可維護的架構。這次我拿 2020 年寫這篇文章當例子 (購物車折扣計算) 來測試一下, 該怎樣做才能讓 coding agent 能做好這任務｡ 我分別扮演兩種角色: PM (假設他不會 coding) vs 架構師, 對比看看架構師多做了什麼, 以及從成果來看有哪些差異｡ 最後結論是: 兩種測試 AI 都能寫出正確的 code，但「正確的規格」仍需要有經驗的人來定義與驗證, 架構師的這組實驗, 明顯對程式碼品質的掌控高得多, 不只是 code, 連同架構設計, 邊界設計, 技術規格, 測試驗證範圍等都有明顯差別。

我想表達的是, 你要有系統的讓 AI 知道你的 "理想" 標準是什麼, AI 才有可能辦的到｡ 在社群上其實沒看到太多這類的案例, 因此這次我就自己動手做了一次...
...
理想的做法, TDD 的第一步應該是先想好 contract, 然後在你實作 contract 之前, 就該先想像 contract 未來會怎麼被使用, 按照這想法先把 test 寫下來, 這就是所謂的 "先寫測試再寫程式” 的精髓｡ 現在既然 contract 都準備好了, 那就來寫 test 吧. 

還記得之前我寫的這篇 vibe testing 嗎? 如果你的介面都設計好了, 你可以很容易的就用 decision table 來決定你測試的範圍, 這是很有效率的圈訂測試範圍的技巧, 我花了一點 "口舌” (tokens) 跟 ai 溝通我要的測試範圍, 過程我省掉了, 我直接貼最終的結果:
...
// TODO: 整合以下三段修訂版內容，形成完整的心得
// 主軸: AI Coding 時代，架構師的價值何在？IDE/Debugger 還需要嗎？
// 1. 黑箱驗證的限制 → 核心邏輯仍需 debugger trace
// 2. Code 作為 Spec → Contract-First 的價值
// 3. 切割邊界的能力 → 架構師的核心技能
// 4. Nathan Sobo (Zed) 的觀點 → IDE 未來的樣貌
// 5. 結論: 別太早丟掉 IDE，但要選對 Agent 友善的工具
