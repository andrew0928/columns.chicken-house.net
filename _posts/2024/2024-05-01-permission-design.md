...
IIdentity 告訴所有開發人員，不管你怎麼管理權限，你就是透過這個介面得知現在是 "誰" 在使用你的系統 (這就是認證的範圍)。而 IPrincipal, 則是回答你這個 "人" 被賦予什麼 "身分"，而若這個 "身分" 可以直接對應到 "允許做什麼事" 的話，那他就是授權的源頭了。

先不管你是怎麼被授權的，你可以用這段 code 取得目前執行你程式碼的 "身分"。如果是 console application (來源可能是 windows 整合式認證), 你可以從綁定目前執行緒的環境取得這兩個介面實作:
...
在這邊，我自己做了一層抽象化，所有的權限管控問題，只有一句話:

> 回答: {你是誰} 能不能夠執行 {你想做什麼} 這件事? 可以就 return true, 不可以就 return false, 或是 throw SecurityException();

如果把它定義成 C# interface:  IPermission, 那大概會是這個樣子:
...
實際舉幾個例子，如果我想實作一個 XXX 系統，只有主管可以查閱機密資訊 (例如業績，統計資料，每筆訂單資訊)，而操作員只能輸入資料 (已知訂單 ID 的 CRUD)，那麼這段 code 寫起來大概像這樣:

//


當然，你 C# 熟練一點，也許可以讓它更好用一點.. (其實 CAS 的設計很讚，只是從 .NET Core 開始就被移除了)
...
OK, 到這邊構思的準備作業大致都完成了。我們有 interface 了，也有對應的 unit test 了。現在什麼 code 都沒有，只會得到一片紅燈而已隨著管理需求不同，案例我會持續擴充，有不同的管理需求才會衍生出不同的管理體系，我會用這方式逐步帶出 RBAC / PBAC / ABAC 的適用情境。

接下來的段落，我會說明這些情境 (會擴充 test case)，並且補上合適的設計 (會實作 interface)，最後讓這些測試都變成綠燈，就算完成目標。
...
// 怎麼做 -> 權限管控要做什麼 (認證 & 授權)
// 怎麼管 -> 資料怎麼處理, 背後的定義與結構 (管理機制, RBAC / PBAC / ABAC)
