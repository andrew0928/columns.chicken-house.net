...
我從開始寫第一行程式碼 ```Hello World, Andrew!``` 開始，所有交給電腦處理的事情都是非常明確的，一個指令一個動作。直到現在，電腦都不大能處理邏輯不明確的操作，這 20 年來最大的差別就是操作變簡單了，但是要明確的指令這件事一直都沒變.. 這也難怪, 電腦從一開始被發明出來, 英文是 "computer" (計算機), 而不是像中文這樣翻譯成 電 "腦", 或是 "智慧" 等等這類用語，計算機就是計算，再複雜的計算還是計算，不是智慧，也不是真人。
...
## 3-3, AI 應用的開發框架 (Semantic Kernel)
...
## 4-3, UI 應該盡可能拆解, 讓 Copilot 能引導操作
...
## 4-4, 挑選正確地軟體開發框架，累積能被 AI 運用的服務與元件
...
##  5-2, 熟悉開發 AI 應用程式必要的 Tech Stack
