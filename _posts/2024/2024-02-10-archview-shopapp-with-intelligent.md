...
上一篇文章 (架構師觀點: XXXXX)，展示了我嘗試的 GPTs 整合應用，實現了讓 AI 助理的嘗試，我開始真的可以用自然語言就完成整個購買流程的操作了。過程中，AI 也幫我 "腦補" 了部分我 API 沒有提供的功能 (指定預算跟購買條件，AI 自動幫我思考購買組合)。這結果其實比我預期的還理想，完成之後，我開始探索接下來的這兩個問題:

- 未來的應用程式，會怎麼運用 "AI" ?
- 未來軟體開發的流程與角色，會變成甚麼樣貌?

一個是未來的軟體執行方式，改變的是使用者使用軟體服務的習慣；另一個是未來軟體開發的架構，流程與分工，改變的是軟體開發領域的生態，包含服務，結構，開發框架，流程，工具等等，預期都會被 AI 翻了一輪。改變是必然的，不確定的是會怎麼變? 因此，我決定延續 "安德魯小舖" 這 PoC, 假設這是五年後的服務呈現的樣貌，那麼這過渡期間會如何發展? 如果五年後是這樣，那麼現在大家使用的工具與知識會有什麼變化?

所以，接下來我會分成幾篇，就來聊聊這些議題，跟我自己的想法吧! 我會附上這陣子我看的文章 & 影片，也會附上我的想法。後半段就當作我的讀書心得吧，而前半段，我就先來交代一下我做這個 PoC 背後的想法，以及實做出來的 source code 與呈現出來的結果。
...
於是，我拆解了這四個部分，重作了一次這題目，作為這篇 demo 的主軸。我認為應用程式若要包含 "智慧" 的話，大致上可以分成這幾個階段:

1. **文字介面: 最基本的選單式的操作**  
(如同那些跛腳的 chet bot 應用, 或是電話語音客服, 要你按數字鍵那種應用), 這當作對照組, 完全不需要 LLM 的應用程式基底。
1. **從操作意圖評估風險**  
在關鍵功能 (我的定義: 結帳) 執行時，能靠 LLM 歸納與推理的能力提供個人化的建議，找出各種不是和交易執行的問題或是風險。
1. **從操作歷程判定使用者的狀況，即時提供操作建議**  
範圍再擴大一點，應用程式可以在背後持續收集使用者的行為，不斷地送給 LLM (就像 APP 不斷地在跟 Chat GPT 講話一樣), 讓 LLM 回報使用者的行為是否有需要留意或是提示 (最典型的案例就是 GitHub Copilot, 用過的都知道我的意思)
1. **全對話式的操作**  
最終極的，才是像我在 "安德魯小舖" GPTs 展示的一樣，完全都透過 Agent 的介面，用自然語言溝通 (包含語音等等媒介)，或是間接由其他系統產生出來的自然語言提示來驅動。當 AI 運算大量普及，運算成本大幅降低時，這是有可能的。

其實，這些發展的脈絡，是我自己先有個構想，在找資料的過程中，看到這篇文章，才發現我想的跟 Microsoft 想的是同一回事，只是人家整理得清楚多了，我就借用一下:
...
如果以完整行為能力的 agent 當作目標的話，如果範圍縮小到只有商店內的操作 (掛進 GPTs 的 API)，並且只需要搭配 Chat GPT 內建的知識 (包含我能上傳的少量 docs) 的話，我先前試作的 "安德魯小舖" GPTs 就是個雛型了。

但是在那之前，應該還有幾個階段才對，案這篇文章標示的，我自己對應了一下:
...
正好，這四個階段，我都有準備範例程式，各位可以試著體驗看看。我的目的在驗證可行性，不過我的腦袋思考能力有限，因此我這次是盡力排除了所有的工程細節 (所謂工程細節: 就是我看技術或是規格，確認已經能辦得道或是能改善的部分)，因為這些工程細節，對我來說是看完文件照著操作，花一些心思除錯就能克服的，這些事情我能等到以後專案確定要執行之前再來克服 (也就是: 把事做對)；現在該做的是先想清楚這件事該不該做 (也就是: 做對的事)。如果有聽過我在 .NET Conf 2023 講的那個場次的話，我講的 "降級" 思考，就是指這麼一回事。
...
而實做過程中，有些我認為該用，但是我還沒足夠熟練度用上的部分:
...
過去，尤其像我這類，在軟體開發領域算是資深的人員，往往所有事情背後都會被拆解成明確的規則，流程，演算法，資料結構等等，然後試圖在所有問題背後都能理出一個脈絡，所有的 input 跟 output 都應該是明確的.. 但是 AI 出現後，尤其是生成式 AI，這一切開始沒有這麼 "明確" 了。連很多 AI 的專家其實也猜不透 LLM 背後運行的邏輯是什麼，只知道餵給他的訓練資料，產出的模型能回答出理想的答案而已。

在這基礎的觀念被打破的時候，你的 "傳統" 應用程式，該如何加入這些 "不可預測" (你無法預期使用者會問你什麼怪問題)，卻又要精準執行 (你要解讀使用者的需求後，呼叫對應的 API，給對應的參數) 系統功能之間的矛盾，就變成這些資深人員需要思考的事情了。

把事情想通了才能正確地應用，這一直是我做事的原則。因此，除了速成的 step by step 之外，重要的領域我會多花一些時間理解這些步驟背後的脈絡，因為想通了我才能跳出 example 的示範，才能融會貫通到各種領域的應用。

因此，這一段，我想在往下看 demo 前，先整理一下思路，如果你看了一堆 API 或是 Framework 後仍然想不通該怎麼應用的話，可以停下來消化一下這一段。

先說我看完的結論，我很佩服 Open AI 開出這些規格 ( Chat Completion, 或是 2023/11 才出來的 Assistant API ) 的人，也很佩服設計出 Semantic Kernel 框架的人。這些 API / Framework 背後，代表的是它們已經把應用程式該怎麼應用 AI 的結構都想通了，並且抽象化變成 API / Framework。各位你該看的不是規格，而是整個應用結構。若套句我在 .NET Conf 2023 的那場，其實就是他們已經把 AI 這領域運作的應用模型都想清楚了。看懂它們的模型，就看得懂它們的 API 跟 SDK。

在這應用模型內，AI 要解決問題，我自己解讀可以拆成這幾塊來看:

### LLM: (模擬人類) 智慧的核心

這邊指的，是純語言模型，沒有任何狀態。基本上就是餵給他一段提示 (prompt), 他就會給你一個回應。而關鍵是 LLM 有足夠的理解能力，你可以當作他真的聽懂了，然後在他被訓練的知識內能回給你 (他認為) 正確的答案。而語言的表達能力是 LLM 內建的，基本上你可把它當作正常人來對談。
