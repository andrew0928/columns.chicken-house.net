...
這是跟前面其他折扣比起來，是個相對複雜的 case, 不過你只要能在 RuleBase 的規範下寫出實作，掛上主程式，其實一點都不難。這邊有些技巧 (商品標籤的標示方式) 要想一下，還有那個折扣規則包含加價可以搭配高一階的商品也要留意...，其他就沒什麼困難的了。這題目是考驗你能不能完全掌握抽象化的思考模式，來擴充折扣規則? 如何在不改變整體架構前提下，把這個複雜的折扣機制實作進去，只要能想通這部分，你大概能將客戶講得清楚的所有折扣都寫出來了吧!

另一個例子是反面的 (沒辦法只靠擴充規則): 如果我的折扣規則，已經跨出我抽象化的那個邊界了呢? 舉例來說，我如果想要指定某些商品，在享用 A 折扣後，如果還符合 B 折扣活動，我想要讓他折上加折是否有辦法? 或是我摳一點，已經享有 A 折扣之後，即使符合條件，你也必須放棄 B 折扣的優惠... 這題目則是考驗，你能不能分得清楚，框架下的每個類別負責的職責是什麼? 該是擴充規則的，就應該在擴充規則的範圍內來擴充，反之若是這框架本身的修正，則應該回到核心來處理，不應在外圍設計一堆 workround 來處理。
...
開始來挑戰複雜的折扣規則擴充練習了，這關能過大概不用擔心了，所有折扣你應該都寫得出來。回頭來看一下，便利商店玩的餐餐超值配，那些 39 / 49 / 59 鮮食 + 飲料的搭配...

老實說要全部寫成規則很麻煩啊，有潔癖的我看到這種規則就開始皺眉頭，我把前面展開的組合再貼一次:
...
討厭的地方在 49 / 59 的鮮食可以跨區使用，廣告的傳單上面也怕顧客看不懂，底下寫了一行字:

> (49 元貼標商品可加 10 元升級搭配 59 元飲品)  
> (可跨區任選 49、59 元鮮食，以咖啡或飲品價格決定餐促售價)

寫這句話的人一定有開發系統的經驗，這規則第二句，比起第一句還更容易寫成 code，他至少很明確的標示計算方式了啊，第一句是寫給人看的，第二句才是開發規格...

真正要開發這則扣規則之前，首先要先解決標記商品資訊的問題。我定義了這幾組 tags:
...
如果商品都標記好了，折扣計算就很單純了，先從價格低的開始 (39 -> 49 -> 59), 能抓到一對的就給優惠價, 抓剩的就抓下一組, 湊不成對的就用原價，每組都優先從原價價格較高的開始湊。
...
老規矩，沒幾行 code, 大家就試著從 source code 去了解我怎麼計算折扣吧。我宣告了一個陣列，來描述配對折扣的規則。飲料的 tag + 鮮食的 tag = 折扣價格，這三個一組的資訊列出陣列就當作我的計算規則。掃出所有結帳的購物車內所有的商品資訊，沒有被排除折扣的東西都挑出來，按照飲料與鮮食來分類並且照價格排序，一一配對並計算折扣。

為了跑這個測試，最花時間的地方不是寫 code 啊，而是建立這些資料跟一個一個去查售價 (全ㄨ網站竟然沒有售價可以查，害我找半天...)。我把商品的資訊列一下:
...
看起來沒錯，總之我就是湊了 49 + 59 兩分，結帳金額 49 + 59 = 108 正確無誤。


接下來，挑戰一下跨區配對。我改一下購買清單:
...
我把 49 那對的飲料，改成 59 的 "單品咖啡 熱美式小杯"，理論上應該照 59 來計算，買下來是 59 + 59 = 118 才對，但是沒改過的 code 不認得跨區的規則，湊不成對的就當元價計算了。結帳金額變成: $143.00, 明細如下:
...
果然便利商店的東西沒有折扣不要買 (咦? 這啥鳥結論...)，沒有折扣就變成 $143.00 了... 不過這結帳程式有幫我重新調整一下配對，至少他把比較貴的單品咖啡拿來湊一對了，如果真的照我自己腦袋的配法，可能結帳就要 39 + 50 + (39 + 45 => 59) = 148... 省了 $5 元不無小補...


接下來我們就調整一下，把規則改成能計算跨區配對的版本吧。這跨區配對規則討厭的地方是，還有例外!! 仔細研究一下規則，39元的飲料跟鮮食是不能跨區配的，只有 49 / 59 可以... 這實在很討厭啊! 我決定把複雜度稍微往前移一點，在資料定義的時候就解決掉他。複雜度加在程式碼身上，會加重往後的除錯困難，我把它擺在最後的選項。

跨區的規則，既然要移到資料定義的部份解決，我有兩個選擇: 一個是程式碼前面的折扣配對規則，另一個是直接標在商品的標籤身上。
...
另一種作法，則是在商品標籤上，把鮮食同時標上 #超值配鮮食49 跟 #超值配鮮食59 兩組標籤，折扣配對的陣列則恢復原狀。這個版本結帳一樣是 $118.00 金額正確，明細如下:
...
從明細可以看到，三明治跟飯糰的商品資訊，後面都帶著兩組標籤 (49 / 59)。結帳程式自然幫我挑出正確的配對了。
...
我埋的這兩個難題，都是實際上會碰到的挑戰。這段落我們接著來看看 "折扣排除" 這需求吧! 這需求困難的地方在於:

我們作抽象化，目的就是讓折扣規則彼此之間盡量獨立，不會互相影響，這樣才能夠獨立開發運作。不過就 "折扣排除" 這需求，擺明了就是折扣之間必須有交互運作才辦的到啊! 這是第一個跟原始假設有衝突的地方。

解決的方式其實也不難，找個方法明確定義排除的規則，然後大家都遵守就好。這邊就衍生出解決問題的策略為何了。所謂的 "排除" 其實有幾種做法，重點在於 "決定權" 在誰身上，這會影響實作的規範該如何定義。我試著列三種可能:

1. 由前面的折扣規則來決定:  
由前面的規則來決定，後面的規則是否會被排除。前面的規則的開發人員，要撰寫是否要跳過後面折扣的邏輯。  
舉例來說，如果 A 折扣很優惠，優惠到只要你套用到 A 折扣，就必須排除後面其他 **所有** 折扣，那就屬於這類。

1. 由後面的折扣決定:
由後面的規則來決定，每次規則套用前，先檢查前面的規則是否有包含互斥的規則? 後面規則的開發人員，要撰寫檢查前面規則的邏輯。  
...
訂定規範的方式，我挑選了一個最直覺的方式: 計算折扣的過程中，標上排除的標籤。

如果每個商品身上，都可以讓各種折扣在處理過程中標上標籤，那麼這些標籤就能附加在商品身上，並且從這個 Rule 傳遞到下一個 Rule 了。如果我定義 "獨佔折扣" 的標籤，並且規定折扣規則必須這樣配合:

1. 如果商品上沒有 "獨佔折扣" 的標籤，那就可以套用該折扣，並且在套用後在該商品上標上 "獨佔折扣" 的標記。
1. 如果商品上有 "獨佔折扣" 的標籤，那麼就必須略過該折扣的優惠。

抽象化思考的用途，再度在這邊發揮效用了。這些規則是很通用的，並且沒有綁在任何特定的規則身上，因此我們有機會把他 "抽象" 抽離到上層類別，以及外圍系統 (本例的 POS) 身上。我再進一步定義:
...
這邊我追加了 field, 定義了 ```ExclusiveTag```, 若他的值不是 ```<NULL>```, 則代表這個折扣規則屬於 "獨佔折扣"。若有多個折扣規則都有同樣的 ```ExclusiveTag```, 則這些折扣只能套用一個。至於誰先誰後，則按照折扣處理的順序為準。

為了避免 ```RuleBase``` 還要小心的挑掉需要排除的商品，我調整了 ```CartContext``` 的設計:
...
追加了 method: ```GetVisiblePurchasedItems()```, 讓 ```RuleBase``` 傳入自己的 ```ExclusiveTag``` 就能拿到排除後的購物清單, 因此原本的 ```RuleBase``` 衍生類別都對應的做了點調整，從原本的:
...
有改變的部分，就是處理完折扣後，額外追加一段，把所有符合折扣的商品，通通補上當前 ```Rule``` 的 ```exclusiveTag``` 作為標記；標記完成後，下一個 ```Rule``` 自然就知道要避開他了。這整套機制 (標記 + 避開) 的協作，就分別由 ```POS``` 跟 ```CartContext``` 負責了。我如果把整套系統的設計一分為二，一個是折扣機制的框架開發團隊，另一個是折扣規則的開發團隊，那麼這些異動，就正巧都很精巧的封裝在框架的開發團隊端，完全不影響規則開發團隊端的運作。除了配合這次改版必要的調整之外，既有規則調整升及後就不需要再顧慮排除的規則。
...
看的到 3 + 5 = 8 串衛生紙，其中 6 串已經符合 "滿件折扣1 (衛生紙滿6件折100)" 優惠，同時其中 5 串衛生紙同時符合兩種折扣，在後面的 "同商品加購優惠 (加10元多一件)" 折扣活動中，又湊齊了兩組，因此最後結帳金額是 $2187.5 元。


接著，商店發現這樣折扣過頭了，店長下令 "滿件折扣1 (衛生紙滿6件折100)" 跟 "同商品加購優惠 (加10元多一件)" 不得重複優惠，若同時符合條件以前面的為準。我們調整一下 code, 只要在 ```LoadRules()``` 這邊替這兩個折扣規則，標上 ```ExclusiveTag``` = "ex"  :
...
接著再跑一次結帳，這次發現結帳金額變成 $2327.50 了，我們看看明細 (我略過購買商品的部分):
